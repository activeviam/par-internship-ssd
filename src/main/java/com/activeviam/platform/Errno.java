...
public class Errno {
  ...
  public static final String[] ERROR_NUMBERS =
      new String[] {
  ...
  public static void throwError(
      final int errno, final String nativeMethodName, final Object... arguments) {
    throw new RuntimeException(
        "We encountered error number "
            + errno
            + (errno < ERROR_NUMBERS.length
                ? (" (" + ERROR_NUMBERS[errno] + ")")
                : "(Have a look at /usr/include/asm-generic/errno.h)")
            + " when performing native call "
            + nativeMethodName
            + "("
            + join(", ", arguments)
            + ")");
