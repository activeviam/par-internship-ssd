...
/**
 * A heap with the ability to store primitive doubles. A heap is a tree-based data-structure
...
public class MinHeapDoubleWithIndices implements IPrimitiveIterator {
	 ...
	 */
	public MinHeapDoubleWithIndices(int capacity) {
	 ...
	 *
	 * @param value the new double to add
	 ...
	 */
	public void add(final double value, final int index) {
		final int n = queue.length - 1;
		int pos = this.size;
		// Bubble up
		while (pos > 0) {
			final int parent = (pos - 1) >>> 1;
			final double p = queue[n - parent];
	...
	public double poll() {
		...
		final double result = queue[n];
		final double last = queue[n - size];
		...
		if (size != 0) {
			...
			while (idx < parentOfLast) {
				...
				double c = queue[n - toPromoteChild];
	...
	public ITransientVector getUnderlyingArray() {
		return new ArrayDoubleVector(queue);
	}
