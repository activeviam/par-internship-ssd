...
{	
	...
	for (uint8_t i = 0; i < chunk->local_cache.actual_size; i++) {
		...
		printf("\t\t line %u block %u address %lx ", i, chunk->local_cache.ids[i], buf);
...
static int
load_lb(struct ssd_chunk *chunk, uint32_t id)
{
	struct ssd_chunk_cache *cache = &chunk->local_cache;
...
fetch_lb(struct ssd_chunk *chunk, uint32_t new_id)
{
	...
	if (new_cacheline == cache->actual_size) {
		...
		int rc = force_cache_update(chunk, new_id);
		...
		chunk->local_cache.curr_cacheline = new_cacheline;
	}
	
	chunk->local_cache.pending[old_cacheline] = 1;
	uint64_t upd = old_cacheline;
	upd << 48;
	upd |= (uint64_t)cache;
	int rc = spdk_nvme_ns_cmd_write(chunk->ns->ns,
								chunk->qpair,
								old_lb,
								chunk->lb_offset + old_id,
								1,
								complete,
								(void*)upd,
								0);
	if (rc != 0) {
		fprintf(stderr, "async ssd write operation failed\n");
		return NULL;
	}

	return ssd_cache_get_page(chunk->global_cache, chunk->local_cache.lbs[new_cacheline]);
}
